...
if arg_dict["split_dataset"]:
    all_data = load_json(os.path.join(arg_dict["data_dir"], "opensource_sample_500.json"))
    all_data = instruction_format(all_data)
    EVAL_NUM = arg_dict["eval_num"]
    random.shuffle(all_data)
    write2json(all_data[:-EVAL_NUM], os.path.join(arg_dict["data_dir"], "opensource_train.json"), "opensource train data")
    write2json(all_data[-EVAL_NUM:], os.path.join(arg_dict["data_dir"], "opensource_dev.json"), "opensource dev data")
...
args = [
    ...
    "--test_file", os.path.join(arg_dict["data_dir"], "{}_dev.json".format(dataset_name)),
    "--output_dir", outdir,
...
def get_metrics_results(eval_preds):
    ...
    def reconstruct_predictions(eval_preds):
        ...
        eval_res = []
        for raw_data, pred in zip(test_data_raw, decoded_preds):
